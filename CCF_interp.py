...
def xcor(t1, y1, t2, y2, tlagmin1, tlagmax1, tunit, imode=0):
    ...
    while tau < tlagmax+safe:
        ...
        if knot>0:
            ...
            ccf12.append(r)
            #taulist.append(tau) --- old version 
            taulist1.append(tau)  #Jennifer Li's edits
            npts.append(knot)
            npts1.append(knot)
        tau += tunit
    # now interpolate 1
    ...
    while tau < tlagmax+safe:
        ...
        if knot>0:
            ...
            if rd1*rd2==0.0:
                ...
                r = (fn*y1y2sum - y2sum*y1sum)/(rd1*rd2)
            ccf21.append(r)
            taulist2.append(tau)   #Jennifer Li's edits
            npts2.append(knot)
        tau += tunit
    ...
    if imode==0:
        #Edits by Jennifer Li to fix the issue of ccf12 having a different length than ccf21 sometimes.
